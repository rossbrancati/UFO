...
filepath = ['/Users/rossbrancati/Desktop/post_control_data/P86/UFO_p86_SPPB_0004.mat'];
load(filepath);

%change the mat_file name for each subjects mat file
mat_file = UFO_p86_SPPB_0005;
...
%subject 65 fell during post single leg, so only sampling first 25 seconds
%here
%raw_COP_x = raw_COP_x(1,1:31374);
%raw_COP_y = raw_COP_y(1,1:31374);
...
%marker locations
firstmet = [(mean(mat_file.Trajectories.Labeled.Data(3,2,:))), mean(mat_file.Trajectories.Labeled.Data(3,1,:))];
secondmet = [(mean(mat_file.Trajectories.Labeled.Data(4,2,:))), mean(mat_file.Trajectories.Labeled.Data(4,1,:))+60];
fifthmet = [mean(mat_file.Trajectories.Labeled.Data(5,2,:)), mean(mat_file.Trajectories.Labeled.Data(5,1,:))];
latheel = [mean(mat_file.Trajectories.Labeled.Data(8,2,:)), mean(mat_file.Trajectories.Labeled.Data(8,1,:))];
heel = [mean(mat_file.Trajectories.Labeled.Data(6,2,:)), mean(mat_file.Trajectories.Labeled.Data(6,1,:))];
medheel = [mean(mat_file.Trajectories.Labeled.Data(7,2,:)), mean(mat_file.Trajectories.Labeled.Data(7,1,:))];
