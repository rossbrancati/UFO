...
filepath = ['/Users/rossbrancati/Desktop/post_control_data/P86/UFO_p86_SPPB_0003.mat'];
load(filepath);

%change the mat_file name for each subjects mat file
mat_file = UFO_p86_SPPB_0004;

%cop_x and cop_y calculations
...
%lmedheel = [mean(mat_file.Trajectories.Labeled.Data(8,2,:)), mean(mat_file.Trajectories.Labeled.Data(8,1,:))];
%llatheel = [mean(mat_file.Trajectories.Labeled.Data(7,2,:)), mean(mat_file.Trajectories.Labeled.Data(7,1,:))];
%r1stmet = [mean(mat_file.Trajectories.Labeled.Data(1,2,:))+40, mean(mat_file.Trajectories.Labeled.Data(1,1,:))];

%left leg forward
l1stmet = [mean(mat_file.Trajectories.Labeled.Data(11,2,:)), mean(mat_file.Trajectories.Labeled.Data(11,1,:))];
rlatheel = [mean(mat_file.Trajectories.Labeled.Data(8,2,:)), mean(mat_file.Trajectories.Labeled.Data(8,1,:))];
rmedheel = [mean(mat_file.Trajectories.Labeled.Data(7,2,:)), mean(mat_file.Trajectories.Labeled.Data(7,1,:))];
...
%8 Markers
%bound_pts = [ltoe; lmedheel; r5thmet; rlatheel; rheel; rmedheel; llatheel; l5thmet];
%6 Markers
%bound_pts = [ltoe; r5thmet; rlatheel; rheel; llatheel; l5thmet];
%4 markers
...
%8 Boundaries
%bound_labels = {'FR','FR','BR','B','BL','BL','FL','F'}; %labels for boundaries
%6 Boundaries
%bound_labels = {'FR','R','B','BL','BL','FL'};
%4 boundaries:
